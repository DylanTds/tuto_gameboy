...
INT8 fallSpeed;        // vitesse de deplacement vertical (saut / chute)
...
void Fall(){
    ...
    fallSpeed = -gravity; // 0 -4 -8 .. -12 -16 -20 -24 ...
    
    if( (canPlayerMove(playerXY[0], playerXY[1]-fallSpeed)) ){
        playerXY[1] -= fallSpeed;
        scroll_sprite(0, 0, -fallSpeed);
    }
    else if( (fallSpeed%8) ){
        ...
        playerXY[1] -= (fallSpeed-4);
        scroll_sprite(0, 0, -(fallSpeed-4));
        jumping = 0;
        falling = 0;
        fallSpeed = 0;
    }
...
void Jump(){
    ...
    if(jumping == 0){
        jumping = 1;
        fallSpeed = playerDim[1]+gravity;// 8 + 4 = 12
    }

    // on reduit a chaque boucle la "vitesse de chute" pour faire une jolie parabole lors du saut
    fallSpeed -= gravity; // 8 4 0 -4 -8 .. -12 -16 -20 -24 ...
    
    //if( canPlayerMove(playerXY[0], playerXY[1]-fallSpeed) ){} elle est a refaire ! ça ne va pas avec les deplacement de plus ou moin de 8px
    if( fallSpeed > 0 ){
        playerXY[1] -= fallSpeed;
        scroll_sprite(0, 0, -fallSpeed);
    }
